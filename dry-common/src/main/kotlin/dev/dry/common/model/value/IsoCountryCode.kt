package dev.dry.common.model.value
...
enum class IsoCountryCode(
    ...
    AG("AG", "ATG", 28, "Antigua and Barbuda"),
    ...
    BT("BT", "BTN", 64, "Bhutan"),
    BO("BO", "BOL", 68, "Bolivia (Plurinational State of)"),
    BQ("BQ", "BES", 535, "Bonaire, Sint Eustatius and Saba"),
    BA("BA", "BIH", 70, "Bosnia and Herzegovina"),
    BW("BW", "BWA", 72, "Botswana"),
    BV("BV", "BVT", 74, "Bouvet Island"),
    BR("BR", "BRA", 76, "Brazil"),
    IO("IO", "IOT", 86, "British Indian Ocean Territory (the)"),
    BN("BN", "BRN", 96, "Brunei Darussalam"),
    ...
    VG("VG", "VGB", 92, "Virgin Islands (British)"),
    ...
    companion object {
        private val byCountryName: Map<String, IsoCountryCode> = values().associateBy { it.countryName }
        private val byAlphaCode: Map<String, IsoCountryCode> =
            values().associateBy { it.alpha2Code } + values().associateBy { it.alpha3Code }

        fun fromAlphaCode(alphaCode: String): IsoCountryCode? = byAlphaCode[alphaCode]
        fun fromCountryName(countryName: String): IsoCountryCode? = byCountryName[countryName]

        fun fromAlphaCodeOrCountryName(alphaCodeOrCountryName: String): IsoCountryCode? {
            if (alphaCodeOrCountryName.length > 3) {
                return fromCountryName(alphaCodeOrCountryName)
            }
            return fromAlphaCode(alphaCodeOrCountryName) ?: fromCountryName(alphaCodeOrCountryName)
        }

        fun fromAlphaCodeOrCountryName(
            alphaCodeOrCountryName: String,
            defaultIsoCountryCode: IsoCountryCode
        ): IsoCountryCode = fromAlphaCodeOrCountryName(alphaCodeOrCountryName) ?: defaultIsoCountryCode
    }
}
